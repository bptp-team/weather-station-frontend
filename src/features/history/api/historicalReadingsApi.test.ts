...
describe("fetchHistoricalReadings", () => {
  it("requests the selected station and date range", async () => {
    ...
    expect(fetchMock.mock.calls[0][0].toString()).toContain(`from=${encodeURIComponent(from.toISOString())}`);
  });

  it("rejects HTTP errors and invalid payloads", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response("", { status: 422 }));
    await expect(fetchHistoricalReadings("station-01", new Date(), new Date())).rejects.toThrow("422");

    vi.restoreAllMocks();
    vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify([{ nope: true }]), { status: 200 }));
    await expect(fetchHistoricalReadings("station-01", new Date(), new Date())).rejects.toThrow("invalid");
  });
});
